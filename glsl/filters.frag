...
// copied and modified from AwesomeBump 5.0:
// https://github.com/kmkolasinski/AwesomeBump/blob/Release/Sources/resources/shaders/filters.frag
...
mat3 sobel_kernel = mat3(-1.0, 0.0, +1.0, -2.0, 0.0, +2.0, -1.0, 0.0, +1.0);
...
subroutine(filterModeType) vec4 mode_perspective_transform_filter() {
    ...
    if (uv_scaling_mode == 1) {
        ...
        // calculating proper mapping for UV coordinates. I used here
        // pow(x,alpha) function
...
subroutine(filterModeType) vec4 mode_invert_components_filter() {
    ...
    vec4 inversion;
    if (gui_image_type == 3) {
        inversion = vec4(gui_inverted_components.r, gui_inverted_components.r,
                         gui_inverted_components.r, 0);
    } else {
...
subroutine(filterModeType) vec4 mode_ao_cancellation_filter() {
    ...
    return (colorA + (1 - colorB) * gui_ao_cancellation) /
           (1 + (1 - colorB.a) * gui_ao_cancellation);
}
...
subroutine(filterModeType) vec4 mode_gauss_filter() {
    ...
    if (gui_gauss_mask == 0) { // standard processing without masking
        ...
        // mask texture
        float mask =
            abs(float(gui_gauss_invert_mask) - clamp(texture(layerB, v2QuadCoords.xy).r, 0, 1));
...
#define UPDATE_SOBEL_VALUES(x, y, sobelMat)                                                        \
    (sobelMat)[0] +=                                                                               \
        textureOffset(layerA, v2QuadCoords.st, ivec2((x), (y))) * sobel_kernel[(x) + 1][(y) + 1];  \
    (sobelMat)[1] +=                                                                               \
        textureOffset(layerA, v2QuadCoords.st, ivec2((x), (y))) * sobel_kernel[(y) + 1][(x) + 1];

    mat2x4 sobel = mat2x4(0);

    // Unrolled 2D loop over offset values
    // Using unrolled loop + macro function, because the offset parameter of the
...
subroutine(filterModeType) vec4 mode_seamless_linear_filter() {
    ...
    if (gui_seamless_mode == 0) {
        ...
        float bomega =
            clamp(omega + omega * pow(length(hA.xyz - hB.xyz) / sqrt(2.0) * cst, pwr), 0, 1);
...
subroutine(filterModeType) vec4 mode_seamless_filter() {
    ...
    if (gui_seamless_mode == 2) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            highp float alpha = 1 - smoothstep(gui_seamless_random_inner_radius,
                                               gui_seamless_random_outer_radius, dist);

            // vec4 hB = texture( layerB, rot_mat*((2.5*cst+0.0)*(tc -
            // atom_pos))+0.5);
        ...
        if (rr > 0) {
            ...
            float rands[4] =
                float[](gui_seamless_random_angles[0][2], gui_seamless_random_angles[1][2],
                        gui_seamless_random_angles[2][2], gui_seamless_random_angles[2][1]);
            ...
            for (int i = 0; i < 4; i++) {
                ...
                weight += alpha;
                vec4 ncolor =
                    texture(layerA, (rot_mat * (2.5 * cst * (angle * 0 + tc - atom_pos)) + 0.5));
                color += ncolor * alpha;
            }
        } // end of if rr
        // return color;
...
float color_dist(vec3 colorA, vec3 colorB) {
    ...
    // modyfications
    if (gui_gray_scale_range_tol == 0.0)
        return 1.0;
    ...
    // return clamp(cdist * gui_gray_scale_range_tol/10.0,0,1); // another
    // example weight
}
...
subroutine(filterModeType) vec4 mode_gray_scale_filter() {
    ...
    color = vec4(1) * (color.r * gui_gray_scale_preset.r + color.g * gui_gray_scale_preset.g +
                       color.b * gui_gray_scale_preset.b);

    // Additional filtering, for base map conversion enabled
    // based on selected by user min and max values force the colors which are
    // "near" them to be 0 (for min color) or 1 (for max color). By near we mean
    // if the calculated distance between two vec3 is close to zero.
...
float rand() { return fract(sin(dot(v2QuadCoords.xy, vec2(12.9898, 78.233))) * 43758.5453); }
...
subroutine(filterModeType) vec4 mode_normal_mixer_filter() {
    vec2 newTexCoords =
        vec2(v2QuadCoords.xy - 0.5) - vec2(gui_normal_mixer_pos_x, gui_normal_mixer_pos_y);
    // rotate
    ...
    // rotate normals
    normalB =
        vec3(normalB.x * cos(-gui_normal_mixer_angle) + normalB.y * sin(-gui_normal_mixer_angle),
...
subroutine(filterModeType) vec4 mode_sobel_filter() {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            sobel_x +=
                texture(layerA, v2QuadCoords.xy + vec2(i - 1, j - 1) * dxy).r * sobel_kernel[i][j];
            sobel_y -=
                texture(layerA, v2QuadCoords.xy + vec2(i - 1, j - 1) * dxy).r * sobel_kernel[j][i];
        }
    }
...
subroutine(filterModeType) vec4 mode_normal_expansion_filter() {
    ...
    if (gui_combine_normals == 0) {
        ...
        float slope =
            (1.0 / (exp(+10 * gui_mix_normals * length(n.xy)) + 1)); // slope based blending param
...
float g_intensity = gui_ssao_intensity; // intensity of the effect
float g_scale = 5 * gui_ssao_depth;     // length of the blast radius
float g_bias = 0.05 * gui_ssao_bias;    //  controls the width of the occlusion
                                        //  cone considered by the occludee.
// Downloading the text item in the eye layout
vec3 getPosition(vec2 uv) { return vec3(uv, -g_bias * texture(layerA, uv).r); }
// Normal download
vec3 getNormal(vec2 uv) { return normalize(texture(layerB, uv).xyz - 0.5); }
...
subroutine(filterModeType) vec4 mode_occlusion_filter() {
    vec3 p = getPosition(v2QuadCoords.xy); // position in the eye system
    vec3 n = getNormal(v2QuadCoords.xy);   // normal
    float ao = 0.0f;
...
subroutine(filterModeType) vec4 mode_height_processing_filter() {
    ...
    vec4 hmin =
        height_clamp(vec4(0.0), vec4(0.0), gui_height_proc_min_value, gui_height_proc_max_value);
    vec4 hmax =
        height_clamp(vec4(1.0), vec4(1.0), gui_height_proc_min_value, gui_height_proc_max_value);
    if (gui_height_proc_normalization) {
        return clamp(vec4(height - hmin) / (hmax - hmin) + vec4(gui_height_proc_offset_value),
                     vec4(0), vec4(1));
    } else {
...
subroutine(filterModeType) vec4 mode_roughness_color_filter() {
    ...
    if (gui_roughness_color_method == 1) { // angle based A
        ...
        vec3 maskColor =
            normalize(gui_roughness_picked_color - 0.5); // normalized value of picked color
...
void main() {
    ...
    int materialIndex = int(256 * 256 * 256 * materialColor.r) + int(256 * 256 * materialColor.g) +
                        int(256 * materialColor.b);
